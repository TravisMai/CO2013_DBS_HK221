...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 24, 2022 at 03:22 PM
...
--
-- Triggers `seasontrainee`
--
DELIMITER $$
CREATE TRIGGER `at_most_three_season` AFTER INSERT ON `seasontrainee` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `at_most_three_season_update` AFTER UPDATE ON `seasontrainee` FOR EACH ROW BEGIN
	DECLARE Total_year int;
	SELECT count(s.Syear) INTO Total_year FROM seasontrainee s WHERE s.Ssn_trainee = new.Ssn_trainee GROUP BY s.Ssn_trainee;
    IF (Total_year >= 3) THEN
   		SIGNAL SQLSTATE '45000' SET
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `join_debut_night` AFTER INSERT ON `seasontrainee` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `join_debut_night_update` AFTER UPDATE ON `seasontrainee` FOR EACH ROW BEGIN
	DECLARE Debut_season year;
    DECLARE errorMessage text;
    SELECT s.SYear INTO Debut_season FROM stageincludetrainee s
    WHERE s.Ep_No = 5 and s.Ssn_trainee = new.Ssn_trainee;
...
--
-- Triggers `stageincludetrainee`
--
DELIMITER $$
CREATE TRIGGER `Insert_total_vote` AFTER INSERT ON `stageincludetrainee` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `Update_total_vote` AFTER UPDATE ON `stageincludetrainee` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `group_stage_check` AFTER INSERT ON `stageincludetrainee` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `group_stage_check_update` AFTER UPDATE ON `stageincludetrainee` FOR EACH ROW BEGIN
	DECLARE group_stage_joined int(3);
    SELECT count(sit.Stage_No) INTO group_stage_joined
    FROM stageincludetrainee sit join stage s
    ON (sit.SYear, sit.Ep_No, sit.Stage_No) = (s.Syear, s.Ep_No, s.Stage_No)
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `stage_five_check` AFTER INSERT ON `stageincludetrainee` FOR EACH ROW BEGIN
	DECLARE group_stage_joined int(3);
